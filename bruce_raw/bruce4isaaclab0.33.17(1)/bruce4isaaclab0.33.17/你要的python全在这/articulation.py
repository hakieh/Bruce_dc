...
class Articulation(AssetBase):
    ...
    def _create_buffers(self):
        ...
        soft_limit_factor = self.cfg.soft_joint_pos_limit_factor
            #bruce limit
        if self._data.default_joint_limits[0,:,0].equal(self._data.default_joint_limits[0,:,1]):
            data = torch.zeros_like(self._data.default_joint_limits)
            data[:,:,0] = -2
            data[:,:,1] = 2
            self._data.default_joint_limits = data
            self._data.joint_limits = data
            self.write_joint_limits_to_sim(data)
            joint_pos_limits = self.root_physx_view.get_dof_limits()
            joint_pos_mean = (joint_pos_limits[..., 0] + joint_pos_limits[..., 1]) / 2
            joint_pos_range = joint_pos_limits[..., 1] - joint_pos_limits[..., 0]
            soft_limit_factor = self.cfg.soft_joint_pos_limit_factor
